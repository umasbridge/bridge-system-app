...
export function useRichText(options: UseRichTextOptions): UseRichTextReturn {
  ...
  const updateSelectionState = useCallback(() => {
    ...
    // Show format panel if text is selected
    if (hasSelection) {
      setShowFormatPanel(true);
    }
  }, [checkHyperlinkAtCursor]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (features.allowHyperlinks && onHyperlinkClick) {
      ...
      if (target.tagName === 'A') {
        ...
        if (pageId) {
          const linkMode = (target.getAttribute('data-link-mode') || 'popup') as 'popup' | 'split' | 'newpage';
          onHyperlinkClick({ pageId, pageName: target.textContent || '', mode: linkMode, position: { x: e.clientX, y: e.clientY } });
        } else {
          // Fallback: parse href (format: "bridge://pageId/mode")
          const href = target.getAttribute('href') || '';
          if (href.startsWith('bridge://')) {
            const parts = href.replace('bridge://', '').split('/');
            onHyperlinkClick({ pageId: parts[0], pageName: target.textContent || '', mode: (parts[1] || 'popup') as 'popup' | 'split' | 'newpage', position: { x: e.clientX, y: e.clientY } });
          }
        }
      }
    }
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (target.tagName === 'A') {
      const href = target.getAttribute('href') || '';
      if (href.startsWith('bridge://') || target.getAttribute('data-page-id')) {
        e.preventDefault();
      }
    }
  }, []);
  ...
  const openHyperlinkMenu = useCallback(() => setShowHyperlinkMenu(true), []);
